...
export async function GET() {
  ...
  const integrations: IntegrationStatus[] = [
    ...
    },
    {
      ...
      configured: !!getEnv("GSC_SITE_URL"),
      properties: getEnv("GSC_SITE_URL") ? [getEnv("GSC_SITE_URL")!] : [],
      missing: getEnv("GSC_SITE_URL") ? [] : ["GSC_SITE_URL"],
      notes: getEnv("GSC_SITE_URL") ? "URL da propriedade configurada." : "Configure GSC_SITE_URL com a propriedade do Search Console (ex: sc-domain:suno.com.br).",
    },
    {
      id: "bigquery",
      title: "BigQuery",
      configured: !!(getEnv("GCP_PROJECT_ID") && getEnv("BIGQUERY_DATASET_ID") && getEnv("GOOGLE_APPLICATION_CREDENTIALS_JSON")),
      properties: [],
      missing: [
        ...(getEnv("GCP_PROJECT_ID") ? [] : ["GCP_PROJECT_ID"]),
        ...(getEnv("BIGQUERY_DATASET_ID") ? [] : ["BIGQUERY_DATASET_ID"]),
        ...(getEnv("GOOGLE_APPLICATION_CREDENTIALS_JSON") ? [] : ["GOOGLE_APPLICATION_CREDENTIALS_JSON"]),
      ],
      notes: "Export raw de eventos GA4. Ativar GA4 → BigQuery Linking primeiro.",
    },
    {
      id: "monday",
      title: "Monday.com",
